...
@interface TWTRImageLoader () <TSEImageDownloader>
...
- (id<NSCopying>)fetchImageWithURL:(NSURL *)url completion:(TWTRImageLoaderFetchCompletion)completion
{
    ...
    [self fetchCachedImageWithImageKey:imageKey cacheHitCompletion:completion cacheMissCompletion:^{
...
- (void)fetchImageWithImageURL:(NSURL *)imageURL requestID:(id<NSCopying>)requestID completion:(TWTRImageLoaderFetchCompletion)completion
{
    ...
    NSURLSessionTask *fetchTask = [self.URLSession dataTaskWithURL:imageURL completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        if ([data length] == 0 || localizedError) {
            dispatch_async(dispatch_get_main_queue(), ^{
                [[Twitter sharedInstance].scribeSink didEncounterError:localizedError withMessage:@"Failed to load image."];
            });
...
#pragma mark - TSEImageDownloader Protocol Methods

- (id)downloadImageFromURL:(NSURL *)URL completion:(TSEImageDownloadCompletion)completion
{
