...
{
    _URL = [[aDecoder decodeObjectForKey:TWTRCardEntityURLEncodingKey] copy];
    return [super init];
}

- (void)encodeWithCoder:(NSCoder *)aCoder
{
    [aCoder encodeObject:self.URL forKey:TWTRCardEntityURLEncodingKey];
}
...
{
    [coder encodeObject:self.imageURL forKey:TWTRCardEntityImageValueImageURLKey];
    [coder encodeCGSize:self.imageSize forKey:TWTRCardEntityImageValueImageSizeKey];
}

- (instancetype)initWithCoder:(NSCoder *)coder
{
    NSString *URL = [coder decodeObjectForKey:TWTRCardEntityImageValueImageURLKey];
    CGSize size = [coder decodeCGSizeForKey:TWTRCardEntityImageValueImageSizeKey];
    return [self initWithURL:URL height:size.height width:size.width];
}
...
{
    return [self.imageURL isEqualToString:other.imageURL] && CGSizeEqualToSize(self.imageSize, other.imageSize);
}
...
- (id)transformedValue:(NSDictionary *)value
{
    if ([value isKindOfClass:[NSDictionary class]]) {
        NSDictionary *userValue = [TWTRDictUtil dictForKey:TWTRCardEntitySiteValueTransformerUserValueKey fromDict:value];
        return [TWTRDictUtil stringForKey:TWTRCardEntityUserValueIDStringKey fromDict:userValue];
    }
    return nil;
}
...
{
    if ([value isKindOfClass:[NSDictionary class]]) {
        return [TWTRDictUtil stringForKey:TWTRBindingValueTransformerStringValueKey fromDict:value];
    }
    return nil;
}
...
{
    if ([value isKindOfClass:[NSDictionary class]]) {
        NSDictionary *imageDict = [TWTRDictUtil dictForKey:@"image_value" fromDict:value];
        NSString *URL = imageDict[@"url"];
        CGFloat height = [imageDict[@"height"] floatValue];
        CGFloat width = [imageDict[@"width"] floatValue];

        if (URL && height > 0 && width > 0) {
            return [[TWTRCardEntityImageValue alloc] initWithURL:URL height:height width:width];
        }
    }
    return nil;
}
