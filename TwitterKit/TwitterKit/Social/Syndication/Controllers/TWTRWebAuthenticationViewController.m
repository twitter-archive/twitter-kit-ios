...
#import <TwitterCore/TwitterUserAPIClient.h>
#import "TWTRErrors.h"
...
@property (nonatomic, readonly) TwitterUserAPIClient *APIClient;
...
- (instancetype)initWithAuthenticationToken:(NSString *)token authConfig:(TWTRAuthConfig *)authConfig APIServiceConfig:(id<TWTRAPIServiceConfig>)serviceConfig hasExistingSession:(BOOL)hasExistingSession
{
    ...
    if (self) {
        ...
        _APIClient = [[TwitterUserAPIClient alloc] initWithAuthConfig:authConfig authToken:token authTokenSecret:nil];
...
{
    if (_useWebFlow) {
        [[Twitter sharedInstance].scribeSink didStartWebLogin];
        return [self webViewController];
    } else {
        [self.navigationController setNavigationBarHidden:YES];
        [[Twitter sharedInstance].scribeSink didStartSafariLogin];
...
- (void)requestAccessTokenWithVerifier:(NSString *)verifier
{
    ...
    NSURLRequest *request = [self.APIClient URLRequestWithMethod:@"POST" URL:postURL.absoluteString parameters:parameters];
...
- (void)succeedWithSession:(TWTRSession *)session
{
    if (self.completion) {
        ...
        if (_useWebFlow) {
            [[Twitter sharedInstance].scribeSink didFinishWebLogin];
        } else {
            [[Twitter sharedInstance].scribeSink didFinishSafariLogin];
        }
...
- (void)failWithError:(NSError *)error
{
    if (self.completion) {
        ...
        if (error.code == TWTRLogInErrorCodeCancelled) {
            if (_useWebFlow) {
                [[Twitter sharedInstance].scribeSink didCancelWebLogin];
            } else {
                [[Twitter sharedInstance].scribeSink didCancelSafariLogin];
            }
        } else {
            if (_useWebFlow) {
                [[Twitter sharedInstance].scribeSink didFailWebLogin];
            } else {
                [[Twitter sharedInstance].scribeSink didFailSafariLogin];
            }
        }
