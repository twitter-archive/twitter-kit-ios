    ...
    TWTRTweetActionAPIClientCompletion clientRequestCompletion = ^(TWTRAPIClient *client, NSError *error) {
        ...
        TWTRTweetActionCompletion requestCompletion = ^(TWTRTweet *tweet, NSError *likeError) {
            ...
            if (tweet || alreadyLiked) {
                ...
                // scribes
                if (attemptingToLike) {
                    [[Twitter sharedInstance].scribeSink didLikeTweetWithID:tweet.tweetID forUserID:tweet.perspectivalUserID fromViewName:self.scribeViewName];
                } else {
                    [[Twitter sharedInstance].scribeSink didUnlikeTweetWithID:tweet.tweetID forUserID:tweet.perspectivalUserID fromViewName:self.scribeViewName];
                }
 ...
 *
 *  @warning This method has to be called on the main thread because of `+[Twitter sharedInstance]`
 */
- (void)APIClientFromTwitterWithCompletion:(TWTRTweetActionAPIClientCompletion)completion
{
    ...
    if (perspectivalUserID && [[Twitter sharedInstance].sessionStore sessionForUserID:perspectivalUserID] != nil) {
        client = [[TWTRAPIClient alloc] initWithUserID:perspectivalUserID];
        completion(client, nil);
    } else {
        BOOL isLoggedIn = ([Twitter sharedInstance].sessionStore.session.userID != nil);
        ...
        if (isLoggedIn) {
            ...
            [[Twitter sharedInstance] logInWithCompletion:^(TWTRSession *session, NSError *error) {
                if (session) {
                    ...
                    NSError *unauthenticatedError = [NSError errorWithDomain:TWTRAPIErrorDomain code:TWTRAPIErrorCodeNotAuthorizedForEndpoint userInfo:@{ NSLocalizedDescriptionKey: @"Endpoint requires user context. Please see -[Twitter logInWithCompletion:]." }];
...
- (void)updateToLikedStateWithAnimation
{
    ...
    [TWTRButtonAnimator performAnimationType:type onButton:self completion:^{
        [self updateToLikedState];
    }];
}
