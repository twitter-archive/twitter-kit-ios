...
#import "TSEConfigurationSelectionTableViewCell.h"
...
@interface TSETweetComposerTableViewDataSource ()

@property (nonatomic, nullable, readonly, copy) TSETweet *initialTweet;
@property (readonly, nonatomic) BOOL allowsAccountSelection;
@property (readonly, nonatomic) BOOL allowsGeoTagging;

// Type: [NSNumber<TSETweetComposerTableViewDataSourceCellType>]
@property (nonatomic) NSArray<NSNumber *> *cellTypes;

@property (nonatomic, nullable, weak) TSEConfigurationSelectionTableViewCell *accountSelectionCell;
@property (nonatomic, nullable, weak) TSEConfigurationSelectionTableViewCell *locationSelectionCell;

@end

@implementation TSETweetComposerTableViewDataSource

- (instancetype)initWithConfiguration:(TSETweetShareConfiguration *)config allowsGeoTagging:(BOOL)allowsGeoTagging
{
    NSParameterAssert(config);

    if ((self = [super init])) {
        _initialTweet = [config.initialTweet copy] ?: [TSETweet emptyTweet];
...
- (void)createCellTypes
{
    ...
    if (self.allowsAccountSelection) {
        [types addObject:@(TSETweetComposerTableViewDataSourceCellTypeAccountSelector)];
    }

    if (self.allowsGeoTagging ) {
        [types addObject:@(TSETweetComposerTableViewDataSourceCellTypeLocationSelector)];
    }
...
{
    return (TSETweetComposerTableViewDataSourceCellType)[self.cellTypes[(NSUInteger)indexPath.row] unsignedIntegerValue];
}

- (void)registerCellClassesInTableView:(UITableView *)tableView
{
    [tableView registerClass:[TSEConfigurationSelectionTableViewCell class] forCellReuseIdentifier:TSEConfigurationSelectionTableViewCell.reuseIdentifier];
}
...
- (void)_tseui_updateTweet:(void (^_Nonnull)(TSETweet *tweet))updateTweet
{
...
- (void)_tseui_updateTweetText:(NSString *)updatedText textSelection:(NSRange)textSelection allowUndo:(BOOL)allowUndo
{
    ...
    if ((updatedText || existingText) && updatedText != existingText && ![updatedText isEqualToString:existingText]) {
        ...
        [self _tseui_updateTweet:^(TSETweet *tweet) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    switch ([self cellTypeAtIndexPath:indexPath]) {
        case TSETweetComposerTableViewDataSourceCellTypeAccountSelector: {
            TSEConfigurationSelectionTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:TSEConfigurationSelectionTableViewCell.reuseIdentifier forIndexPath:indexPath];
...
#pragma mark - TSETweetTextViewContainerDelegate
