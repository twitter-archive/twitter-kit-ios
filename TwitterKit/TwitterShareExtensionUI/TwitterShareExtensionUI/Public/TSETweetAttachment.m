...
NSString * const TSETweetShareExtensionErrorDomain = @"TSETweetShareExtensionErrorDomain";


#pragma mark -

@implementation TSETweetAttachmentImage
...
@implementation TSETweetAttachmentURL
...
@interface TSETweetAttachmentCocoaItemProvider ()
@property (nullable, nonatomic, readwrite) NSString *urlString;
@end

@implementation TSETweetAttachmentCocoaItemProvider {
    dispatch_queue_t _afterURLLoadQueue;
    NSMutableArray<TSETweetAttachmentAfterURLLoadBlock> *_afterURLLoadBlocks;
}
              ...
              imageDownloader:(nullable id<TSEImageDownloader>)imageDownloader
{
    ...
    if ((self = [super init])) {
        ...
        if ([_itemProvider hasItemConformingToTypeIdentifier:(NSString *)kUTTypeURL]) {
            ...
            [_itemProvider loadItemForTypeIdentifier:(NSString *)kUTTypeURL options:nil completionHandler:^(NSURL *url, NSError *error) {
                ...
                if (strongSelf) {
                    ...
                    __block NSArray<TSETweetAttachmentAfterURLLoadBlock> *afterURLLoadBlocks;
                    ...
                    for (TSETweetAttachmentAfterURLLoadBlock afterURLLoadBlock in afterURLLoadBlocks) {
                        afterURLLoadBlock();
                    }
                }
            }];
