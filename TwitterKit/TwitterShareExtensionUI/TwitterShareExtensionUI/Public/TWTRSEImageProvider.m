...
- (void)_tseui_loadWithOptions:(NSDictionary *)options
                          mode:(TWTRSEItemProviderLoadImageMode)mode
                       success:(TWTRSEImageSuccessBlock)successBlock
                       failure:(TWTRSEImageInternalFailureBlock)failureBlock
{
    ...
    switch (mode) {
        ...
        default:
        {
            ...
            failureBlock([NSError errorWithDomain:TWTRSETweetShareExtensionErrorDomain
...
- (void)loadWithOptions:(NSDictionary *)options success:(TWTRSEImageSuccessBlock)successBlock failure:(TWTRSEImageFailureBlock)failureBlock
{
    ...
    void (^loadImageTypeBlock)(NSError * _Nullable) = ^(NSError * _Nullable loadObjectOfClassError) {
        [self _tseui_loadWithOptions:options mode:TWTRSEItemProviderLoadImageModeLoadImageType success:^(UIImage *_Nonnull typeLoadImage) {
            successBlock(typeLoadImage);
        } failure:^(NSError * _Nullable loadImageTypeError) {
            [self _tseui_loadWithOptions:options mode:TWTRSEItemProviderLoadImageModeLoadFileURLType success:^(UIImage *_Nonnull typeLoadFileURLImage) {
    ...
    if (@available(iOS 11, *)) {
        [self _tseui_loadWithOptions:options mode:TWTRSEItemProviderLoadImageModeLoadImageClass success:^(UIImage * _Nonnull classLoadImage) {
            successBlock(classLoadImage);
        } failure:^(NSError * _Nullable loadImageClassError) {
            [self _tseui_loadWithOptions:options mode:TWTRSEItemProviderLoadImageModeLoadFileURLClass success:^(UIImage * _Nonnull classLoadFileURLImage) {
