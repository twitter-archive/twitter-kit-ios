...
- (instancetype)initWithInReplyToTweetID:(NSNumber *)inReplyToTweetID text:(NSString *)text attachment:(id<TSETweetAttachment>)attachment place:(id<TSEGeoPlace>)place usernames:(NSArray<NSString *> *)usernames hashtags:(NSArray<NSString *> *)hashtags
{
    if ((self = [super init])) {
...
- (nonnull NSString *)textWithLeadingUsernames:(nullable NSArray<NSString *> *)usernames hashtags:(nullable NSArray<NSString *> *)hashtags text:(nullable NSString *)text
{
    ...
    for (NSString *username in usernames) {
        [presentationText appendFormat:@"%@ ", TSEDisplayUsername(username)];
    }
...
{
    return [[TSETweet alloc] initWithInReplyToTweetID:self.inReplyToTweetID text:self.text attachment:self.attachment place:self.place usernames:nil hashtags:nil];
}
