...
- (void)setUp
{
    ...
    [[Twitter sharedInstance].sessionStore saveSession:userSession withVerification:NO completion:^(id<TWTRAuthSession> session, NSError *error){
    }];
}
...
- (void)testComposer_usesText
{
    ...
    // Verify
    [composer showFromViewController:[[UIViewController alloc] init] completion:^(TWTRComposerResult result){
    }];
...
- (void)testShowFromViewController_presentsFromController
{
    ...
    TWTRComposer *composer = [[TWTRComposer alloc] init];
    [composer showFromViewController:mockFromController completion:^(TWTRComposerResult result){
    }];
...
- (void)testShowFromViewController_attemptsLogin
{
    ...
    [[Twitter sharedInstance].sessionStore logOutUserID:@"555"];

    // Expect that login will be called
    id mockTwitter = OCMPartialMock([Twitter sharedInstance]);
    OCMExpect([mockTwitter logInWithCompletion:OCMOCK_ANY]);
    [Twitter setSharedTwitter:mockTwitter];

    // Invoke login call
    TWTRComposer *composer = [[TWTRComposer alloc] init];
    [composer showFromViewController:[UIViewController new] completion:^(TWTRComposerResult result){
    }];
