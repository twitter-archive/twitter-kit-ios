...
- (NSString *)textForTweet:(TSETweet *)tweet;
- (UIImage *)imageForTweet:(TSETweet *)tweet;
...
@property (nonatomic) TSETweet *fakeTweet;
...
- (void)setUp
{
    ...
    self.fakeTweet = [[TSETweet alloc] initWithInReplyToTweetID:nil text:@"Tweet text" attachment:nil place:nil usernames:nil hashtags:nil];
...
{
    TSETweetAttachmentURL *attachment = [[TSETweetAttachmentURL alloc] initWithTitle:@"URL Title" URL:[NSURL URLWithString:@"http://www.fakeurl.com"] previewImage:[[UIImage alloc] init]];
    TSETweet *tweet = [[TSETweet alloc] initWithInReplyToTweetID:nil text:@"Tweet text" attachment:attachment place:nil usernames:nil hashtags:nil];
...
- (void)testImageForTweet_returnsImageIfExists
{
    ...
    TSETweet *tweetWithImage = [[TSETweet alloc] initWithInReplyToTweetID:nil text:@"Tweet text" attachment:[[TSETweetAttachmentImage alloc] initWithImage:image] place:nil usernames:nil hashtags:nil];
...
- (void)testSendTweet_sendsTweetText
{
    ...
    [self.networking sendTweet:self.fakeTweet fromAccount:[TWTRComposerAccount new] completion:^(TSENetworkingResult result){
    }];
...
{
    TSETweet *tweetWithImage = [[TSETweet alloc] initWithInReplyToTweetID:nil text:@"Tweet text" attachment:[[TSETweetAttachmentImage alloc] initWithImage:[TWTRImages verifiedIcon]] place:nil usernames:nil hashtags:nil];

    [self.networking sendTweet:tweetWithImage fromAccount:[TWTRComposerAccount new] completion:^(TSENetworkingResult result){
    }];
...
- (void)testSendTweet_sendsURLInText
{
    ...
    [self.networking sendTweet:tweet fromAccount:[TWTRComposerAccount new] completion:^(TSENetworkingResult result){
    }];
...
- (void)testSendTweet_cancelsPendingVideoUpload
{
    ...
    [self.networking sendTweet:self.fakeTweet fromAccount:self.fakeAccount completion:^(TSENetworkingResult result){
    }];
...
- (void)testSendTweet_callCompletionSuccess
{
    ...
    [self.networking sendTweet:self.fakeTweet fromAccount:[TWTRComposerAccount new] completion:^(TSENetworkingResult result) {
        if (result == TSENetworkingResultSuccess) {
            [expectation fulfill];
        }
    }];
...
- (void)testSendTweet_callCompletionFailure
{
    ...
    XCTestExpectation *expectation = [self expectationWithDescription:@"Called completion"];
    [self.networking sendTweet:self.fakeTweet fromAccount:[TWTRComposerAccount new] completion:^(TSENetworkingResult result) {
        if (result == TSENetworkingResultError) {
            [expectation fulfill];
        }
    }];
