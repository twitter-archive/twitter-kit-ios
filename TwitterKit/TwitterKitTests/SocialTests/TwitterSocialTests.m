...
@property (nonatomic, readonly) Twitter *twitterKit;
...
- (Twitter *)createUniqueInstance
{
    [Twitter resetSharedInstance];
    Twitter *twitter = [[Twitter alloc] init];
    [twitter startWithConsumerKey:@"k" consumerSecret:@"s"];
    XCTAssert([twitter class] == [Twitter class], @"Allocated Twitter object has class %@. If that class is an OCMock class, you probably need to call [twitterMock stopMocking] somewhere", [twitter class]);
    return twitter;
}

- (void)testResetSharedInstance
{
    Twitter *firstTwitter = [Twitter sharedInstance];
    [Twitter resetSharedInstance];
    Twitter *secondTwitter = [Twitter sharedInstance];
...
{
    [self.twitterKit.sessionStore saveSession:self.session withVerification:NO completion:^(id<TWTRAuthSession> _Nullable session, NSError *_Nullable error){
    }];
...
- (void)testNotificationPostedWhenUserLoggedOut
{
    ...
    [[Twitter sharedInstance].sessionStore saveSession:session withVerification:NO completion:^(id a, id b){
    }];

    __block NSString *loggedOutID = nil;

    __weak __block id observer = [[NSNotificationCenter defaultCenter] addObserverForName:TWTRUserDidLogOutNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification *note) {
