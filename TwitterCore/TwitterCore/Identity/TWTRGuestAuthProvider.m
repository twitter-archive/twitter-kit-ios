 ...
 */
@property (nonatomic, strong) TwitterAppAPIClient *appAPIClient;
...
- (instancetype)initWithAuthConfig:(TWTRAuthConfig *)authConfig apiServiceConfig:(id<TWTRAPIServiceConfig>)apiServiceConfig accessToken:(NSString *)accessToken
{
    ...
    if (self = [super init]) {
        ...
        _appAPIClient = [[TwitterAppAPIClient alloc] initWithAuthConfig:authConfig accessToken:accessToken];
    }
...
- (void)authenticateWithCompletion:(TWTRAuthenticationProviderCompletion)completion
{
    ...
    [self.appAPIClient sendAsynchronousRequest:[self activateNewGuestTokenRequest] completion:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
        [[self class] validateResponseWithResponse:response data:data connectionError:connectionError completion:^(NSDictionary *responseDict, NSError *guestAuthError) {
            if (guestAuthError) {
...
- (NSURLRequest *)activateNewGuestTokenRequest
{
    ...
    NSURLRequest *activateNewGuestTokenRequest = [self.appAPIClient URLRequestWithMethod:@"POST" URL:activateNewGuestTokenURL.absoluteString parameters:nil];
