...
@property (nonatomic, readonly) TwitterUserAPIClient *twitterClient;
...
- (instancetype)initWithAuthConfig:(TWTRAuthConfig *)authConfig apiServiceConfig:(id<TWTRAPIServiceConfig>)apiServiceConfig errorLogger:(id<TWTRErrorLogger>)errorLogger
{
    ...
    if (self = [super init]) {
        ...
        _twitterClient = [[TwitterUserAPIClient alloc] initWithAuthConfig:authConfig authToken:nil authTokenSecret:nil];
    }
...
- (void)getAuthTokenWithAccount:(ACAccount *)account completion:(TWTRAuthenticationProviderCompletion)completion
{
    ...
    NSURLRequest *request = [self.twitterClient URLRequestWithMethod:@"POST" URL:postURL.absoluteString parameters:parameters];
